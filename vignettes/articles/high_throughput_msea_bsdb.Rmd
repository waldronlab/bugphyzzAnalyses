...
```{r tms sets}
l <- length(unique(tms$body_site)) * length(unique(tms$rank))
tms_sets <- vector('list', l)
tms_bs <- unique(tms$body_site)
tms_r <- unique(tms$rank)
n <- 1
for (i in seq_along(tms_bs)) {
  for (j in seq_along(tms_r)) {
    names(tms_sets)[n] <- paste0(tms_bs[i], '_', tms_r[j])
    tms_sets[[n]] <- tms |> 
      filter(body_site == tms_bs[i], rank == tms_r[j]) |> 
      pull(taxid) |> 
      unique()
    n <- n + 1
  }
...
glimpse(bp, width = 70)
```
Which attribute groups (also referred to as physiologies in the documentation):

```{r}
whichAttrGrp(bp)
```

Which attributes, which wuold be the names of the signatures:
