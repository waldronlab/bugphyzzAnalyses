...
system.time({
    ...
    for (i in seq_along(body_sites)) {
        ...
        sigList <- bsdb_sigs_BS_gn[[body_sites[i]]]
        res <- map(sigList, ~ microbeSetEnrichment(.x, bk, bp_sigs_gn)) |> 
            formatEnSigs()
...
```{r}
bk_TMS_BSDB_gn <- unique(c(
    unlist(tms_sigs_BS_gn, use.names = FALSE),
    unlist(bsdb_sigs_BS_gn, use.names = FALSE)
))
bk_TMS_BSDB_sp <- unique(c(
    unlist(tms_sigs_BS_sp, use.names = FALSE),
    unlist(bsdb_sigs_BS_sp, use.names = FALSE)
))
...
system.time({
    en_umeta_TMS_BSDB_intersect_gn <- map(
        umetasigs_gn,
        ~ microbeSetEnrichment(.x, bk_umeta_TMS_BSDB_intersect_gn, bp_sigs_gn)
        ) |> 
        formatEnMetSig()
})
```
...
system.time({
    en_umeta_TMS_BSDB_intersect_sp <- map(
        umetasigs_sp,
        ~ microbeSetEnrichment(.x, bk_umeta_TMS_BSDB_intersect_sp, bp_sigs_sp)
        ) |> 
        formatEnMetSig()
})
```
...
system.time({
    ...
    for (i in seq_along(body_sites)) {
        bk <- bk_umeta_TMS_BSDB_BS_intersect_gn[[body_sites[i]]]
        rx <- paste0(body_sites[i], "-%-%-")
        sig_list <- umetasigs_gn[grep(rx, names(umetasigs_gn))]
        res <- map(sig_list, ~ microbeSetEnrichment(.x, bk, bp_sigs_gn)) |> 
            formatEnMetSig()
...
system.time({
    ...
    for (i in seq_along(body_sites)) {
        bk <- bk_umeta_TMS_BSDB_BS_intersect_sp[[body_sites[i]]]
        rx <- paste0(body_sites[i], "-%-%-")
        sig_list <- umetasigs_sp[grep(rx, names(umetasigs_sp))]
        res <- map(sig_list, ~ microbeSetEnrichment(.x, bk, bp_sigs_sp)) |> 
            formatEnMetSig()
