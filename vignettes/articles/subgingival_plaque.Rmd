...
Main goals of this vignette:

+ Find bugphyzz signatures (aerophilicity) enriched in the taxa that
are differentially abundant in the *subgingival* plaque. Expected result:
anaerobic.
+ Find bugphyzz signatures (aerophilicity) enriched in the taxa that
are differentially abundant in the *supragingival* plaque. Expected result:
aerobic.
+ Contrast for DA: subgingival vs supragingival plaque. Subgingival will be
the 'control' or 0, so a negative fold change means more abundant in the 
subgingival plaque and a positive fold change means more abundant in the 
supragingival plaque.
+ Enrihcment using ORA inlcuded in the bc3net and EnrichmentBrowser packages.
...
arrange(both_ora, pval)
```

> When both sets are combined, no significant enrichment is found.
The output is the same as the output of EnrichmentBrowser (below).
...
res <- sbea(
  ...
  alpha = 0.1 # FDR cutoff for DA taxa
