...
mean(as.integer(rownames(tse_subset) %in% unique(unlist(gn_sigs, use.names = FALSE ))))
```
...
mean(as.integer(rownames(tse_subset) %in% unique(unlist(gn_sigs_noasr, use.names = FALSE ))))
```
...
edger <- limmaVoom(edger)
gsea <- sbea(
  method = 'gsea', se = edger, gs = gn_sigs_noasr, perm = 1000, padj.method = 'BH',
  alpha = 0.1
)
gsea_tbl <- as.data.frame(gsea$res.tbl)
gsea_tbl <- gsea_tbl |> 
...
ora_up <- sbea(
  method = 'ora', se = edger_up, gs = gn_sigs_noasr, alpha = 0.1, perm = 0, 
  padj.method = 'BH'
)
ora_up_df <- as.data.frame(ora_up$res.tbl)
ora_up_df$BODY.SITE <- 'Supragingival plaque'
...
ora_down <- sbea(
  method = 'ora', se = edger_down, gs = gn_sigs_noasr, alpha = 0.1, perm = 0, 
  padj.method = 'BH'
)
ora_down_df <- data.frame(ora_down$res.tbl)
ora_down_df$BODY.SITE <- 'Subgingival plaque'
