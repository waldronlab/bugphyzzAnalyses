...
plotEnrichmentRes <- function(res, body_site, tax_level) {
  ...
  p <- dat |>  
    ...
    ggplot(aes(bp_sig, reorder(bugsigdb_sig, -log10(p_value)))) +
      geom_point(
      aes(
        color = -log10(p_value),
    ...
    facet_wrap(~`Abundance in Group 1`, nrow = 2, scales = 'free_y') +
    labs(
      x = 'bugphyzz sets',
...
```{r enrichment - stool - genus, fig.height=8, fig.width=10}
...
```{r enrichment - stool - species, fig.width=10, fig.height=8}
...
```{r enrichemnt - vagina - genus, fig.height=5, fig.width=8}
...
```{r enrichment - vagina - species, fig.height=4, fig.width=8}
...
```{r enrichment - oral cavity - genus, fig.height=5, fig.width=8}
sets_oc_gn <- getSets(
...
```{r enrihcment - oral cavity - species, fig.width=8, fig.height=5}
sets_oc_sp <- getSets(
...
```{r}
p_value_2 <- stats::fisher.test(ct2, alternative = 'g')$p.value
p_value_2
```
...
```{r}
some_taxids <- 
    bsdb_sets_[[taxa_name_]][bsdb_sets_[[taxa_name_]] %in% bp_sigs_gn$`bugphyzz:aerophilicity|aerobic`]

bsdb_taxa <- propagation$aerophilicity |> 
    filter(NCBI_ID %in% some_taxids) |> 
    arrange(NCBI_ID)
bsdb_taxa
```
...
```{r}
some_taxids_2 <- 
    background_sets_[[taxa_name_]][background_sets_[[taxa_name_]] %in% bp_sigs_gn$`bugphyzz:aerophilicity|aerobic`]

propagation$aerophilicity |> 
    filter(NCBI_ID %in% some_taxids_2, !NCBI_ID %in% some_taxids) |> 
    arrange(NCBI_ID)
```
