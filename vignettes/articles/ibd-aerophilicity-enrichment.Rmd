...
x_regex <- paste0('(', paste0(attribute_names, collapse = '|'), ')')

aer_sig <- aer_sig[grep(x_regex, names(aer_sig))]
...
```{r, message=FALSE, warning=FALSE}
aer_plus <- aer_plus[!is.na(aer_plus$Attribute_type),]
aer_plus_sig <- getSignatures(
    aer_plus, tax.id.type = 'NCBI_ID', tax.level = 'species'
)
aer_plus_sig <- aer_plus_sig[grep(x_regex, names(aer_sig))]
...
```{r}
enriched_taxa <- enrich_output_plus$gs$`bugphyzz:aerophilicity|anaerobic`
