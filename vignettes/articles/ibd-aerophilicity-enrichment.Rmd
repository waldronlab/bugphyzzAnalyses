...
tse <- curatedMetagenomicData(
    "HMP_2019_ibdmdb.relative_abundance", dryrun = FALSE, rownames = 'NCBI'
)[[1]]
tse 
```
...
```{r}
tidy_data <- biobroom::tidy.RangedSummarizedExperiment(tse, addPheno = TRUE)
sample_data <- distinct(select(tidy_data, -gene, -value)) 
