...
```{r}
srank <- count(bp_source, Attribute_source, Rank, name = 'n_rank') |> 
  filter(!grepl('BacDive;', Attribute_source))
sphys <- count(bp_source, Attribute_source, Attribute_group, name = 'n_grp') |> 
  filter(!grepl('BacDive;', Attribute_source)) 
...
```{r}
data <- count(bp_source, Attribute_source, Attribute_group, Rank) |> 
  filter(!grepl('BacDive;', Attribute_source)) |> 
  mutate(new_column = paste0(Rank, '|', Attribute_group)) |>
...
```{r}
# lgd2 <- Legend(col_fun = color_fun, title = "# annotations")
ht_sources <- Heatmap(
  matrix = data,
  cluster_rows = FALSE, cluster_columns = FALSE,
  row_names_side = 'left',
...
## Organize this heatmap by physiology rather than rank.
## Use gaps betwen physiologies and use bar for annotation of ranks.
## Use black and one (presence - ausence).
```
