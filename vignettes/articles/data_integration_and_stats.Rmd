...
# color_fun <- circlize::colorRamp2(
#   breaks = c(0, min(m), max(m)), colors = c('white', 'gray20', "gray10")
# )
color_fun <- circlize::colorRamp2(
  breaks = c(0, 1), colors = c('white', "gray20")
)
df <- data.frame(Rank = sub("^(.*\\|)(\\w+)$", "\\2", rownames(m)))
mycols =list(Rank = c("genus" = "orange","species" = "#009E73", "strain" = "#CC79A7"))
row_ha <- rowAnnotation(df = df, col = mycols, width = unit(5, "cm"))
splits_attrs <- sub("^(.*)\\|\\w+$", "\\1", rownames(m))
