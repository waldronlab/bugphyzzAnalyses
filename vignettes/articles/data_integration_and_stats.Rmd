...
```{r columns with NAs, message=FALSE}
...
This includes all ASR output

```{r histogram of scores, message=FALSE}
...
scores_df <- map(bp2_discrete, ~ pull(.x, Score)) |> 
    unlist(recursive = TRUE, use.names = FALSE) |> 
    {\(y) data.frame(x = y)}()
scores_df |> 
    ggplot(aes(x)) +
    geom_histogram(
        binwidth = 0.1, fill = "gray80", color = "black", linewidth = 0.2
    ) +
    labs(
...
nsti_df <- map(bp2_numeric, ~ pull(.x, NSTI)) |> 
    unlist(recursive = TRUE, use.names = FALSE) |> 
    {\(y) data.frame(x = y)}()
nsti_df |> 
    ggplot(aes(x)) +
...
## 10-fold cross-validation and counts per evidence type (species)
...
pValDis <- val |> 
    ...
    geom_hline(yintercept = 0.8, linetype = "dotdash", linewidth = 0.2) +
    geom_point(
        aes(size = ltp_bp),
...
## counts (genus, species, strain) and annotations per taxon
